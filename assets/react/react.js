...
function getTargetIDForInputEventIE(topLevelType, topLevelTarget, topLevelTargetID) {
  if (topLevelType === topLevelTypes.topSelectionChange || topLevelType === topLevelTypes.topKeyUp || topLevelType === topLevelTypes.topKeyDown) {
    ...
    // propertychange on the first input event after settings `value` from a
...
var DOMProperty = {
   ...
   * hasSideEffects:
   *   Whether or not settings a value causes side effects such as triggering
...
var ReactClass = {
  ...
  createClass: function (spec) {
    ...
    // Reduce time spent doing lookups by settings these on the prototype.
...
var ReactCompositeComponentMixin = {
  ...
  /**
   * Processes props by settings default values for unspecified props and
...
function assertValidProps(component, props) {
  ...
  if ("development" !== 'production') {
    "development" !== 'production' ? warning(props.innerHTML == null, 'Directly settings property `innerHTML` is not permitted. ' + 'For more information, lookup documentation on `dangerouslySetInnerHTML`.') : undefined;
  ...
  _updateDOMProperties: function (lastProps, nextProps, transaction, node) {
    ...
    for (propKey in nextProps) {
      ...
      if (propKey === STYLE) {
        ...
        // from the DOM node instead of inadvertantly settings to a string. This
...
/**
 * Implements an <input> native component that allows settings these optional
...
var ReactDOMOption = {
  mountWrapper: function (inst, props, context) {
    ...
    if ("development" !== 'production') {
      "development" !== 'production' ? warning(props.selected == null, 'Use the `defaultValue` or `value` props on <select> instead of ' + 'settings `selected` on <option>.') : undefined;
    }
...
/**
 * Implements a <select> native component that allows optionally settings the
...
/**
 * Implements a <textarea> native component that allows settings `value`, and
...
var ReactDOMTextarea = {
  ...
  mountWrapper: function (inst, props) {
    ...
    if (children != null) {
      if ("development" !== 'production') {
        "development" !== 'production' ? warning(false, 'Use the `defaultValue` or `value` props instead of settings ' + 'children on <textarea>.') : undefined;
      }
...
/**
 * Enqueues settings the markup of a node.
 *
 * @param {string} parentID ID of the parent component.
 * @param {string} markup Markup that renders into an element.
 * @private
...
/**
 * Enqueues settings the text content.
 *
 * @param {string} parentID ID of the parent component.
...
var Mixin = {
  ...
  perform: function (method, scope, a, b, c, d, e, f) {
    ...
    try {
      ...
      // errorThrown set to true before settings it to false after calling
